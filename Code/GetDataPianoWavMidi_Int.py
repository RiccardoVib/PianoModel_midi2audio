...
def create_I_O_data(data_dir, seed=422):
    ...
    I_O = pickle.load(I_O)
    wav = (np.array(I_O['wav']))
    midi = (np.array(I_O['midi']))
...
def get_data(data_dir, seed=422):
    ...
    signals = float2pcm(np.array(wav['signal']))
...
if __name__ == '__main__':
    ...
    file_data = open(os.path.normpath('/'.join([data_dir, 'Dataset_midi_2_wav_int.pickle'])), 'wb')
    ...
    file_data = open(os.path.normpath('/'.join([data_dir, 'Dataset_prepared_32_int.pickle'])), 'wb')
    pickle.dump(data, file_data)
    file_data.close()